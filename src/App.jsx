...
function App() {
  ...
  const handleSignUp = async () => {
    ...
    if (error) {
      ...
      setMessage('Compte créé ! Vérifiez votre e-mail.')
      setScreen('success')
    }
    setLoading(false)
  }
  ...
  if (screen === 'signup') return (
    <div style={{minHeight:'100vh',background:'#f8f5ef',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'2rem',fontFamily:'sans-serif'}}>
      <div style={{background:'white',borderRadius:'20px',padding:'2rem',width:'100%',maxWidth:'380px',boxShadow:'0 8px 32px rgba(0,0,0,0.08)'}}>
        ...
        {message && <p style={{color:'#1a9e6e',fontSize:'0.82rem',marginBottom:'12px'}}>{message}</p>}
  ...
  if (screen === 'success') return (
    <div style={{minHeight:'100vh',background:'#1e2419',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'2rem',fontFamily:'sans-serif',textAlign:'center'}}>
      ...
      <h2 style={{color:'white',fontWeight:'800',marginBottom:'8px'}}>Bienvenue dans HomeStrong !</h2>
      <p style={{color:'rgba(255,255,255,0.5)',marginBottom:'32px'}}>Vérifiez votre e-mail pour confirmer votre compte.</p>
      <button onClick={() => setScreen('signin')}
        style={{background:'#d4e84a',color:'#111',border:'none',borderRadius:'99px',padding:'14px 28px',fontWeight:'600',cursor:'pointer'}}>
        Se connecter →
      </button>
    </div>
  )
  ...
  if (screen === 'dashboard') return (
    <div style={{minHeight:'100vh',background:'#f8f5ef',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'2rem',fontFamily:'sans-serif',textAlign:'center'}}>
      ...
      <h2 style={{fontWeight:'800',color:'#111',marginBottom:'8px'}}>Bonjour {prenom || 'champion(ne)'} 👋</h2>
      <p style={{color:'#6b7a6a',marginBottom:'32px'}}>Votre espace HomeStrong est prêt.</p>
      <button onClick={() => { setScreen('welcome'); setEmail(''); setPassword(''); setPrenom(''); setMessage('') }}
